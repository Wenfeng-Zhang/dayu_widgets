...
class CheckBoxGroupExample(QtWidgets.QWidget, MFieldMixin):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("CheckBoxGroup Example")
        self._init_ui()
    ...
    def _init_ui(self):
        ...
        check_group_h = MCheckBoxGroup()
        check_group_v = MCheckBoxGroup(orientation=QtCore.Qt.Vertical)

        check_group_h.set_button_list(self.app_data)
        check_group_v.set_button_list(self.app_data)

        self.data_list = ["北京", "上海", "广州", "深圳", "郑州", "石家庄"]
        check_group_b = MCheckBoxGroup()
        check_group_b.set_button_list(self.data_list)
        ...
        self.bind(
            "checked_app", check_group_b, "dayu_checked", signal="sig_checked_changed"
        )
        self.bind("checked_app_text", label, "text")

        check_group_tri = MCheckBoxGroup()
        check_group_tri.set_button_list(self.app_data)
        self.register_field("check_grp", ["Maya"])
        self.bind(
            "check_grp", check_group_tri, "dayu_checked", signal="sig_checked_changed"
        )

        main_lay = QtWidgets.QVBoxLayout()
        main_lay.addWidget(MDivider("Orientation Qt.Horizontal"))
        main_lay.addWidget(check_group_h)
        main_lay.addWidget(MDivider("Orientation Qt.Vertical"))
        main_lay.addWidget(check_group_v)

        main_lay.addWidget(MDivider("Data Bind"))
        main_lay.addWidget(check_group_b)
        main_lay.addWidget(label)
        main_lay.addWidget(button)

        main_lay.addWidget(MDivider("Try Context Menu"))
        main_lay.addWidget(check_group_tri)
        main_lay.addStretch()
