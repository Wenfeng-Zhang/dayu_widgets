...
class MPushButtonDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        button.icon = MIcon("app-nuke.png")
        row_height = style.sizeFromContents(QtWidgets.QStyle.CT_ItemViewItem, option, QtCore.QSize()).height()
        button.iconSize = QtCore.QSize(row_height, row_height)
        # button.backgroundBrush = QtGui.QBrush(QtGui.QColor(43, 43, 43, 0))
        # button.text = "Click Me (" + str(index.data(QtCore.Qt.DisplayRole)) + ")"
        button.state = QtWidgets.QStyle.State_Enabled

        color = QtGui.QColor(255, 0, 0)  # Red color
        palette = QtGui.QPalette()
        palette.setColor(QtGui.QPalette.Button, color)
        button.palette = palette

        style.drawControl(QtWidgets.QStyle.CE_PushButton, button, painter)
...
class DelegateButtonExample2(QtWidgets.QWidget):
    def __init__(self, parent=None):
        super(DelegateButtonExample2, self).__init__(parent)
        self._init_ui()
