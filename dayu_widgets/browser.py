...
class FileButton(MToolButton):
    ...
    def __init__(self, text="", multiple=False, parent=None):
        super(FileButton, self).__init__(parent=parent)
        self.setAcceptDrops(True)
        self.setMouseTracking(True)
        self.text_under_icon()
        self.set_dayu_svg("folder_line.svg")
        ...
        self.set_dayu_size(size)
        self.setIconSize(QtCore.QSize(size, size))
        self.setText(text if text else self.tr("Click or drag Path here"))
        self.clicked.connect(self.slot_browser_folder)
        self.setSizePolicy(
            QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding
        )
        self.setToolTip(self.tr("Click to browser Path or drag Path here"))
    ...
    def dropEvent(self, event):
        ...
        if self.get_dayu_multiple():
            ...
            self.sig_file_changed.emit(file_list[0] if file_list else '')
            self.set_dayu_path(file_list[0] if file_list else '')

    def slot_browser_folder(self):
        r_folder = QtWidgets.QFileDialog.getExistingDirectory(self, "Browser Folder", '')
        if not r_folder:
            return
        file_list = []
        if self.get_dayu_filters():
            for f in os.listdir(r_folder):
                new_file_name = os.path.join(r_folder, f)
                if os.path.isfile(new_file_name) and os.path.splitext(f)[-1] in self.get_dayu_filters():
    ...
    def _get_valid_file_list(self, url_list):
        ...
        file_list = []
        for url in url_list:
            file_name = url.toLocalFile().replace('\\', '/')
            if os.path.isfile(file_name):
                if self.get_dayu_filters():
                    if os.path.splitext(file_name)[-1] in self.get_dayu_filters():
                        file_list.append(file_name)
                ...
                if self.get_dayu_filters():
                    for f in os.listdir(file_name):
                        new_file_name = os.path.join(file_name, f)
                        if os.path.isfile(new_file_name) and os.path.splitext(f)[-1] in self.get_dayu_filters():
                            file_list.append(new_file_name.replace('\\', '/'))
                else:
                    for f in os.listdir(file_name):
                        new_file_name = os.path.join(file_name, f)
                        if os.path.isfile(new_file_name):
                            file_list.append(new_file_name.replace('\\', '/'))
            self.sig_path_changed.emit(file_name)
        return file_list
